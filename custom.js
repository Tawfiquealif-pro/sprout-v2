...
try{
    ...
    if (!commonNames || commonNames.length === 0) {
            loadingStatus.innerHTML = '<p>🌍 Fetching local common names based on your location...</p>';
        try {
            ...
            const gptCommonName = await getPlantInfoFromGPT(scientificName, locationName);
            commonNames = gptCommonName;
            loadingStatus.innerHTML = '<p>Fetch success!</p>';
   ...
   let PLANT = {
    ...
    // Displaying plant information
        ...
        plantCommonNamePlaceholder.textContent = commonNames.common_name
        lifeSpanPlaceholder.textContent = commonNames.average_life_span
        classificationPlaceholder.innerHTML = `<ul><li>Family- ${commonNames.classification.family}</li>
                                              <li>Genus- ${commonNames.classification.genus}</li></ul>`
        intrestingFact.textContent = commonNames.interesting_fact
...
async function getPlantInfoFromGPT(scientificName, location) {
  ...
  const prompt = `
      ...
      If any information is not available, respond with "Info unavailable" for that section. Format the response as JSON:
      {
          "common_name": "",
          "classification": {genus: 'genus name', family: 'family name', species: 'species name'},          "average_life_span": "",
          "usually_found_place": "",
          "interesting_fact": ""
      }
  `;
